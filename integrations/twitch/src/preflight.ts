...
export class TwitchPreflight {
  ...
  async prepare(input: { eventId: string; title: string; category?: string }) {
    ...
    const game = games.find(x => x.name.localeCompare(category, undefined, { sensitivity: 'accent' }) === 0);
    ...
    return { status: 'ready' as const, ...desired, unchanged: current.title === desired.title && current.gameId === desired.gameId };
  }
