...
export class DiscordSoundboardProvider {
  ...
  constructor(private configured: boolean, private readonly transport?: DiscordSoundboardTransport) { if (configured) this.lifecycle = integrationState('DISCONNECTED'); }
  state(): IntegrationState { return structuredClone(this.lifecycle); }
  async sounds() { if (!this.configured) throw named('DISCORD_SOUNDBOARD_NOT_CONFIGURED', 'Discord Soundboard n’est pas configuré.'); if (!this.transport) throw named('DISCORD_SOUNDBOARD_NOT_SUPPORTED', 'Cette distribution ne possède pas de transport vocal Discord Soundboard vérifié.'); try { const sounds = await this.transport.list(); this.lifecycle = transitionIntegration(this.lifecycle, 'CONNECTED'); return sounds; } catch (error) { this.fail(error); throw error; } }
