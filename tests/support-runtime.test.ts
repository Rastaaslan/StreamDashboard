...
describe('Support runtime', () => {
  ...
  it('reste NOT_SUPPORTED tant que cette distribution ne fournit aucun transport Streamlabs', async () => { const adapter = new StreamlabsAdapter('', async () => undefined); await adapter.connect(); expect(adapter.state().status).toBe('NOT_SUPPORTED'); adapter.configure('secret-in-memory'); await adapter.connect(); expect(adapter.state().status).toBe('NOT_SUPPORTED'); });
});
