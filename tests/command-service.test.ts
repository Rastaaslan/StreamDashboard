...
describe('service de commandes', () => {
  it('active Chatting en un appui tout en restant dans le mode live', async () => {
    ...
    const obs: ObsCommands = { state: { connected: true, streaming: true, scene: 'Live' }, scene: vi.fn(), mute: vi.fn(), volume: vi.fn(), stream: vi.fn(), record: vi.fn(), restartMedia: vi.fn(), refresh: vi.fn(), waitForScene: vi.fn(async scene => { obs.state.scene = scene; }) };
    const commit = vi.fn(async () => ({}) as never);
    const service = new DashboardCommandService(domain, obs, commit, { settings: { modeScenes: { live: 'Live' }, chattingScene: 'Just Chatting' } });
    await service.execute({ type: 'scene.chatting' });
    expect(obs.scene).toHaveBeenCalledTimes(1); expect(obs.scene).toHaveBeenCalledWith('Just Chatting');
    expect(obs.waitForScene).toHaveBeenCalledWith('Just Chatting'); expect(domain.mode).toBe('live');
    expect(domain.timer).toEqual(beforeTimer); expect(obs.stream).not.toHaveBeenCalled(); expect(commit).toHaveBeenCalledOnce();
  });
