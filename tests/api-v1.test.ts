...
describe('API publique v1', () => {
  ...
  it('refuse une origine WebSocket étrangère', async () => {
    const app = await start();
    await expect(new Promise<void>((resolve, reject) => {
      const ws = new WebSocket(app.url.replace('http:', 'ws:') + '/ws/v1', { origin: 'https://evil.example' });
      ws.on('open', () => reject(new Error('origine étrangère acceptée'))); ws.on('error', () => resolve());
