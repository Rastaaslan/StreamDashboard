...
describe('Android Live domains', () => {
  it('expose Soundboard, Supports, Automations et Diagnostics', () => { for (const panel of ['supports', 'automations']) expect(html).toContain(`data-live-panel="${panel}"`); expect(html).toContain('data-view="sounds"'); expect(html).toContain('Diagnostics développeur · Events'); });
