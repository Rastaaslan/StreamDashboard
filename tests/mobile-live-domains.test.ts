...
describe('Android Live domains', () => {
  it('expose Soundboard, Supports, Automations et Diagnostics', () => { for (const panel of ['soundboard', 'supports', 'automations']) expect(html).toContain(`data-hub-panel="${panel}"`); expect(html).toContain('Diagnostics développeur · Events'); });
