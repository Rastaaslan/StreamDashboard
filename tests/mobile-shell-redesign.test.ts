...
describe('mobile information architecture', () => {
  ...
  it('rend le chat et les contrôles essentiels visibles dans Live', () => { for(const id of ['hub-chat','chat-form','live-viewers','open-scenes-live','quick-mic','live-stream']) expect(html).toContain(`id="${id}"`); expect(html).toContain('Plus d’outils'); expect(html).not.toContain('hub-tool-tabs'); });
  it('réserve Sons à sa destination principale', () => { expect(html).toContain('data-view="sounds"'); expect(html).toContain('id="sound-search"'); expect(html).toContain('id="stop-sound"'); expect(html).not.toContain('data-open-tab="sounds"'); expect(html).not.toContain('data-hub-panel="soundboard"'); });
