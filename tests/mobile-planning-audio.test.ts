...
import { DEFAULT_FILTERS, filterPlanning, periodBounds, weekAgenda } from '../apps/mobile/planning-model.js';
...
describe('planning mobile', () => {
  ...
  it('produit sept blocs lundi à dimanche, y compris les jours sans live', () => {
    const days = weekAgenda([{ title: 'Live mardi', startAtUtc: '2026-09-15T18:00:00Z', category: 'live' }], DEFAULT_FILTERS, new Date(2026, 8, 12, 14));
    expect(days).toHaveLength(7); expect(days[0].date.getDay()).toBe(1); expect(days[6].date.getDay()).toBe(0);
    expect(days.filter(day => day.events.length)).toHaveLength(1); expect(days.filter(day => !day.events.length)).toHaveLength(6);
  });
});
