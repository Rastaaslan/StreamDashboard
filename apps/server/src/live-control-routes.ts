...
export function registerLiveControlRoutes(options: Options) {
  ...
  app.post('/api/v1/soundboard/volume', async (req, res, next) => { try { const volume = Number(req.body?.volume); if (!Number.isFinite(volume) || volume < 0 || volume > 1) throw new Error('Volume soundboard invalide.'); await soundboard.setVolume(volume); res.json(await soundboard.snapshot()); } catch (error) { next(error); } });
  app.post('/api/v1/soundboard/stop', async (_req, res, next) => { try { await soundboard.stop(); res.json(await soundboard.snapshot()); } catch (error) { next(error); } });
