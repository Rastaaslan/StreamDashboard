...
export async function startDashboardServer(options: DashboardServerOptions = {}): Promise<DashboardServerHandle> {
  ...
  const defaults: LocalData = {
    ...
    settings: {
      ...
      startMode: 'intro',
      remoteEnabled: false,
      requireTimerOverlayOnStart: false,
    },
  ...
  local.settings = {
    ...
    primaryMicInput: typeof rawSettings.primaryMicInput === 'string' && rawSettings.primaryMicInput.trim().length <= 200 ? rawSettings.primaryMicInput.trim() || undefined : undefined,
  ...
  const commandResults = new Map<string, Promise<{ command: ReturnType<typeof parseCommand>; state: DashboardState }>>();
  const executeCommand = async (body: unknown, remote = false) => {
    const input = object(body) ? body : {};
    const commandId = typeof input.commandId === 'string' && /^[A-Za-z0-9._:-]{8,128}$/.test(input.commandId) ? input.commandId : null;
    const payload = commandId ? Object.fromEntries(Object.entries(input).filter(([key]) => key !== 'commandId' && key !== 'correlationId')) : body;
    const run = async () => {
      const command = remote ? parseRemoteCommand(payload, snapshot()) : parseCommand(payload);
    ...
    const existing = commandResults.get(commandId); if (existing) return existing;
    const operation = run(); commandResults.set(commandId, operation);
    ...
    try {
      ...
      const state = await configure(async () => {
        ...
        if (input.primaryMicInput !== undefined) {
          if (typeof input.primaryMicInput !== 'string' || input.primaryMicInput.length > 200) throw new Error('Micro principal invalide.');
          const mic = input.primaryMicInput.trim();
          if (mic && obs.state.connected && !Object.hasOwn(obs.state.inputs, mic)) throw new Error('Le micro principal doit être une entrée audio OBS connue.');
          local.settings.primaryMicInput = mic || undefined;
        }
        if (typeof input.requireTimerOverlayOnStart === 'boolean') local.settings.requireTimerOverlayOnStart = input.requireTimerOverlayOnStart;
