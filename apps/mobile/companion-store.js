...
const empty = () => ({ schemaVersion: COMPANION_SCHEMA_VERSION, planning: [], tombstones: [], pending: [], notes: [], checklist: [], templates: [], recentTwitchCategories: [], streamerName: '', preferences: {}, lastServerSyncAt: null });
...
export function createCompanionStore(storage = localStorage, clock = now) {
  ...
  const operation = (type, eventId, baseRevision, patch, desiredPublication) => ({ id: uid('op'), type, eventId, baseRevision, timestamp: clock(), patch: clone(patch || {}), desiredPublication: clone(desiredPublication || {}) });
  const saveEvent = (input, baseRevision) => {
    ...
    data.pending.push(operation(current ? 'update' : 'create', item.id, current?.revision || 0, input, item.desiredPublication));
  ...
  return {
    ...
    replaceServerSnapshot(snapshot) {
      ...
      const deleted = new Set(data.tombstones.map(item => item.eventId));
      ...
      data.checklist = clone(snapshot.checklist || data.checklist); data.streamerName = snapshot.settings?.streamerName || data.streamerName;
