...
export function drawEventCard(ctx, item, card, image, compact = false) {
  ...
  const artWidth = compact ? Math.round(artHeight * .76) : Math.min(116, Math.round(artHeight * .76));
  const art = { x: x + pad, y: y + pad, width: artWidth, height: artHeight, radius: compact ? 11 : 16 };
  drawCoverImage(ctx, image, art);
  const textX = art.x + art.width + (compact ? 14 : 22);
  const textWidth = x + width - pad - textX;
  ctx.fillStyle = '#ffbd85'; setFont(ctx, 900, compact ? 22 : 31); ctx.fillText(eventTime(item), textX, y + (compact ? 33 : 45));
  if (!compact) {
  ...
  const title = fitCanvasText(ctx, item.title || 'Live', { maxWidth: textWidth, maxLines: compact ? 1 : 2, maxSize: compact ? 20 : 28, minSize: compact ? 17 : 22, weight: 800 });
  ctx.fillStyle = '#fff'; setFont(ctx, 800, title.size); drawLines(ctx, title.lines, textX, y + (compact ? 61 : 80), compact ? 23 : title.size + 5);
  const categoryY = compact ? y + height - 15 : y + height - 22;
  ...
  if (weekly) {
    ...
    days.forEach((day, index) => {
      ...
      if (!day.events.length) { ctx.fillStyle = '#706a7d'; setFont(ctx, 550, 19); ctx.fillText('Pas de live prévu', 70, y + 75); }
        ...
        const entries = day.events.slice(0, 2); const gap = 10, available = 102;
        const cardWidth = (960 - gap * (entries.length - 1)) / entries.length;
        entries.forEach((item, eventIndex) => drawEventCard(ctx, item, { x: 60 + eventIndex * (cardWidth + gap), y: y + 32, width: cardWidth, height: available }, images[imageIndex++], true));
