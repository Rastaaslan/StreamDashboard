...
function renderChecklist() {
  ...
  if (onlinePc() && values.length) {
    ...
    reset.type = 'button';
    reset.className = 'secondary checklist-reset';
    reset.onclick = async () => {
      try {
        await ensureCredential();
        const result = await transport.command({ type: 'checklist.reset' });
        applyCriticalState(result.state);
        await syncCompanionNow();
      } catch (error) { notify(error.message); }
    };
...
async function toggleStreamFromPhone(button) {
  button.disabled = true;
  try {
    await ensureCredential();
    ...
    const question = start
      ...
      : current.settings?.confirmStop ? 'Arrêter réellement le live ?' : 'Arrêter le live ?';
    if (!confirm(question)) return;
    notify(start ? 'Démarrage du live…' : 'Arrêt du live…');
    const result = await transport.command({ type: start ? 'session.start' : 'session.stop', ...(start ? { force: false } : {}) });
    if (!start && result.state?.obs?.streaming) throw new Error('OBS indique que le live est toujours actif après la commande d’arrêt.');
    applyCriticalState(result.state);
    notify(start ? 'Live démarré.' : 'Live arrêté.');
  } catch (error) { notify(error.message); }
  finally { button.disabled = !hotState?.obs?.connected; }
}
...
document.addEventListener('click', event => {
  ...
  if (button.id === 'stream') {
    event.preventDefault(); event.stopImmediatePropagation();
    void toggleStreamFromPhone(button);
    return;
  }
