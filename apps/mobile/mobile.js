...
import { createMobileCommandController, shouldApplyState } from './command-controller.js';
import './templates-ui.js';
...
const commandController = createMobileCommandController({ transport, getCredential: () => credential, applyState: next => render(next), notify: note });
...
async function command(value) {
  try {
    const body = await commandController.execute(value);
...
function renderPlanning(items) {
  ...
  for (const item of pagination.items) {
    ...
    if (!item.occurrenceKey) {
      ...
      const edit = text('button', 'Modifier'); edit.type = 'button'; edit.onclick = () => openMobileEditor({ ...item, seriesId: item.id }, 'event');
      const remove = text('button', 'Supprimer'); remove.type = 'button'; remove.onclick = () => void removeMobileSeries({ ...item, seriesId: item.id });
...
function render(next) {
  if (!next) return;
  if (!shouldApplyState(state, next)) return;
  state = next;
  ...
  $('live').textContent = next.obs.streamingKnown === false ? 'ÉTAT INCONNU' : next.obs.streaming ? 'LIVE' : 'OFFLINE';
  $('live').className = next.obs.streamingKnown !== false && next.obs.streaming ? 'ok' : '';
...
async function connect() {
  ...
  try {
    ...
    ws.onclose = () => {
      ...
      // WebSocket is telemetry only. Probe REST before declaring the PC down and
      // preserve the last-known state while realtime reconnects.
      void transport.state({ timeoutMs: 5_000 }).then(next => {
        render(next); setConnectionMode(CompanionMode.ONLINE_PC);
        ...
        if (!state) render(offlineState());
      }).finally(() => { reconnectTimer = setTimeout(connect, retry); retry = nextRetry(retry); });
    };
    ws.onerror = () => ws.close();
  } catch (error) {
    setConnectionMode(resolveMode({ pcAvailable: false, internetAvailable: navigator.onLine }));
...
$('slot-form').onsubmit = async event => {
  ...
  try {
    ...
    } else if (mobileEditing?.scope === 'series' || mobileEditing?.scope === 'event') {
...
async function startLive() {
  ...
  if (!start) { await command({ type: 'session.stop' }).catch(() => undefined); return; }
  try {
    note('Préparation du live…'); await commandController.execute({ type: 'session.prepare' });
    note('Démarrage du live…'); await commandController.execute({ type: 'session.start', force: false });
    note('Live démarré.');
  } catch (error) {
    if (error?.status === 409 && confirm('Checklist incomplète. Démarrer quand même ?')) {
      await commandController.execute({ type: 'session.start', force: true }); note('Live démarré avec confirmation explicite.');
    } else note(error.message);
  }
}
$('stream').onclick = () => void startLive();
...
function renderCompanion() {
  ...
  draw('notes', cache.notes, item => item.text);
  const checklist = $('checklist'); checklist.replaceChildren(...cache.checklist.map(item => { const toggle = text('button', `${item.done ? '✓' : '○'} ${item.label}`); toggle.type = 'button'; toggle.className = `checklist-toggle${item.done ? ' done' : ''}`; toggle.onclick = async () => { try { if (companionMode === CompanionMode.ONLINE_PC) await command({ type: 'checklist.toggle', id: item.id }); else { companion.upsertCollection('checklist', { ...item, done: !item.done }); renderCompanion(); } } catch (error) { note(error.message); } }; return toggle; }));
}
for (const [buttonId, kind, inputId, property] of [['add-note','notes','note-text','text'],['add-check','checklist','check-label','label']]) $(buttonId).onclick = () => { const input = $(inputId); if (!input.value.trim()) return; companion.upsertCollection(kind, { [property]: input.value.trim(), ...(kind === 'checklist' ? { done: false } : {}) }); input.value = ''; renderCompanion(); note('Enregistré localement · À synchroniser.'); };
renderCompanion();
void start();
