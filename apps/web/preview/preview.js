...
function connectRuntimeSocket(){
  ...
  socket.onmessage=event=>{try{const message=JSON.parse(event.data);if(message.type==='state.updated'&&message.data){applyDashboard(message.data);if(!editableFocus())render()}}catch{/* événement invalide ignoré */}};
