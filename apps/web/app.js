...
let page = 'overview', state, remotePairing = null;
...
function toast(text, bad = false) { const element = $('#toast'); if (!element) return; element.textContent = text; element.className = bad ? 'show bad' : 'show'; setTimeout(() => { element.className = ''; }, 2600); }
...
function bindForms() {
  ...
  if (eventForm) eventForm.onsubmit = async event => { event.preventDefault(); const form = new FormData(eventForm); try { const result = await request('/api/v1/planning', 'POST', { title: form.get('title'), startAtUtc: new Date(form.get('start')).toISOString(), endAtUtc: new Date(form.get('end')).toISOString(), category: form.get('category'), twitchCategoryName: form.get('twitchCategoryName') || undefined, desiredPublication: { local: true, twitch: form.get('publishTwitch') === 'on', google: form.get('publishGoogle') === 'on' } }); eventForm.dataset.dirty = 'false'; applyStateUpdate(result, true); $('#event-dialog')?.close(); } catch (error) { toast(error.message, true); } };
  const settingsForm = $('#settings-form');
  if (settingsForm) {
    settingsForm.elements.accent.value = state.settings.accent;
    settingsForm.onsubmit = async event => { event.preventDefault(); const form = new FormData(settingsForm); const payload = { streamerName: form.get('streamerName'), obsExecutablePath: form.get('obsExecutablePath'), accent: form.get('accent'), confirmStop: form.get('confirmStop') === 'on', launchObs: form.get('launchObs') === 'on', remoteEnabled: form.get('remoteEnabled') === 'on', timerBrowserSource: form.get('timerBrowserSource'), obsUrl: form.get('obsUrl'), modeScenes: Object.fromEntries(['intro', 'live', 'pause', 'end'].map(mode => [mode, form.get(`scene-${mode}`)]).filter(([, scene]) => scene)) }; const clearPassword = form.get('clearObsPassword') === 'on', password = form.get('obsPassword'); if (clearPassword) payload.clearObsPassword = true; else if (password) payload.obsPassword = password; try { const result = await request('/api/v1/settings', 'PUT', payload); settingsForm.dataset.dirty = 'false'; applyStateUpdate(result, true); toast(payload.remoteEnabled !== state.remote?.enabled ? 'Réglages enregistrés · redémarrage requis pour le LAN' : 'Réglages enregistrés'); } catch (error) { toast(error.message, true); } };
  }
}
