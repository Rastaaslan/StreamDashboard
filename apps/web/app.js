...
async function refresh(force = false) { const [next, product, connections] = await Promise.all([request('/api/v1/state'), request('/api/v1/profile'), request('/api/v1/connections')]); productProfile = product.profile; connectionProjection = connections.items || []; moduleStates = product.modules; applyProductAppearance(); applyStateUpdate(next, force); }
...
function moduleEnabled(id) { return productProfile?.modules?.[id] !== false; }
const pageModules = { planning: ['planning'], prepare: ['checklist','notes','templates'], live: ['obs','twitch'], deck: ['obs'], sounds: ['soundboard'], fun: ['obs'], supports: ['streamlabs'], automations: ['automations'] };
function pageEnabled(id) { return !pageModules[id] || pageModules[id].some(moduleEnabled); }
function ensureEnabledPage() { if (!pageEnabled(page)) page = 'overview'; }
function applyProductAppearance() { if (!productProfile) return; const appearance = productProfile.appearance; for (const key of ['theme','preset','density','radius']) document.documentElement.dataset[key] = appearance[key]; document.documentElement.style.setProperty('--product-accent', appearance.accent); document.documentElement.style.setProperty('--product-font-scale', appearance.textScale === 'large' ? '1.15' : appearance.textScale === 'small' ? '.9' : '1'); }
function nav() {
  ensureEnabledPage(); const visiblePages = pages.filter(([id]) => pageEnabled(id));
  $('nav').innerHTML = visiblePages.map(([id, label, icon], index) => `<button data-page="${id}" class="${id === page ? 'active' : ''}" title="${label} · Alt+${index + 1}"><span>${icon}</span>${label}</button>`).join('');
  document.querySelectorAll('.secondary-nav [data-page]').forEach(button => { const required = pageModules[button.dataset.page]; button.hidden = required ? !required.some(moduleEnabled) : false; });
...
function connections() {
  ...
  return `<div class="intent-panel"><span class="label">CONNEXIONS</span><h3>Services connectés</h3><div class="connection-cards">
    ...
    ${visible('googleCalendar') ? `<article><div><b>Google Calendar</b><span>${connectionCopy('google')}</span></div>${supportsConnection('google', state.google.connected ? 'disconnect' : 'connect') ? `<button class="secondary compact" data-action="${state.google.connected ? 'disconnect-google' : 'connect-google'}">${state.google.connected ? 'DÉCONNECTER' : 'CONNECTER'}</button>` : ''}</article>` : ''}
    ${visible('discord') ? `<article><div><b>Discord</b><span>${connectionCopy('discord')}</span></div>${supportsConnection('discord', 'configure') ? '<button class="secondary compact" data-action="go" data-value="settings">CONFIGURER</button>' : ''}</article>` : ''}
    ${visible('streamlabs') ? `<article><div><b>Streamlabs</b><span>${connectionCopy('streamlabs')}</span></div>${supportsConnection('streamlabs', 'configure') ? `<form id="streamlabs-form"><input name="token" type="password" maxlength="1000" autocomplete="new-password" aria-label="Token Socket Streamlabs" placeholder="Token Socket API"><div class="button-row"><button class="primary compact" type="submit">CONFIGURER</button><button class="secondary compact" type="button" data-action="test-streamlabs">TESTER</button><button class="danger compact" type="button" data-action="disconnect-streamlabs">DÉCONNECTER</button></div></form>` : ''}</article>` : ''}
    ${visible('wizebot') ? `<article><div><b>WizeBot</b><span>${connectionCopy('wizebot')}${wizebot.profile?.name ? ` · ${esc(wizebot.profile.name)}` : ''}</span></div>${supportsConnection('wizebot', 'configure') ? `<form id="wizebot-form"><input name="apiBaseUrl" type="url" required aria-label="Adresse API fournie par WizeBot" placeholder="Adresse API fournie par WizeBot"><input name="token" type="password" maxlength="1000" required autocomplete="new-password" aria-label="Token WizeBot" placeholder="Token API"><div class="button-row"><button class="primary compact" type="submit">CONFIGURER</button><button class="secondary compact" type="button" data-action="refresh-wizebot">RAFRAÎCHIR</button><button class="danger compact" type="button" data-action="disconnect-wizebot">DÉCONNECTER</button></div></form>` : ''}</article>` : ''}
...
function productSettings() {
  ...
  const a = productProfile.appearance;
  return `<form class="panel settings" id="profile-form"><span class="label">PROFIL STREAMDASHBOARD</span><h3>Profil et apparence</h3><label>Nom affiché<input name="displayName" maxlength="80" value="${esc(productProfile.profile.displayName)}"></label><label>Chaîne<input name="channelName" maxlength="80" value="${esc(productProfile.profile.channelName)}"></label><div class="form-grid"><label>Thème<select name="theme">${['system','light','dark','oled'].map(value => `<option ${a.theme === value ? 'selected' : ''} value="${value}">${({system:'Système',light:'Clair',dark:'Sombre',oled:'OLED'})[value]}</option>`).join('')}</select></label><label>Style<select name="preset">${['minimal','soft','compact','contrast'].map(value => `<option ${a.preset === value ? 'selected' : ''} value="${value}">${({minimal:'Minimal',soft:'Doux',compact:'Compact',contrast:'Contrasté'})[value]}</option>`).join('')}</select></label><label>Couleur principale<input name="accentColor" type="color" value="${esc(a.accent)}"></label><label>Densité<select name="density">${['comfort','normal','compact'].map(value => `<option ${a.density === value ? 'selected' : ''} value="${value}">${({comfort:'Aérée',normal:'Normale',compact:'Compacte'})[value]}</option>`).join('')}</select></label><label>Arrondis<select name="radius">${['square','medium','round'].map(value => `<option ${a.radius === value ? 'selected' : ''} value="${value}">${({square:'Carré',medium:'Léger',round:'Arrondi'})[value]}</option>`).join('')}</select></label><label>Texte<select name="textScale">${['small','normal','large'].map(value => `<option ${a.textScale === value ? 'selected' : ''} value="${value}">${({small:'Petit',normal:'Normal',large:'Grand'})[value]}</option>`).join('')}</select></label></div><details class="advanced-provider-modes"><summary>Mode avancé des connexions</summary><p class="muted">Official utilise la configuration de distribution. Custom est réservé aux forks et au self-hosting.</p><div class="form-grid">${Object.entries(productProfile.providers).map(([id, provider]) => `<label>${id}<select name="provider-${id}"><option value="official" ${provider.mode === 'official' ? 'selected' : ''}>Officiel</option><option value="custom" ${provider.mode === 'custom' ? 'selected' : ''}>Personnalisé</option></select></label>`).join('')}</div></details><span class="label section-label">MODULES</span><div class="module-list">${moduleStates.map(module => `<label class="switch"><span><b>${esc(module.label)}</b><small>${module.dependencies.length ? `Nécessite ${module.dependencies.join(', ')}` : module.capabilities.join(' · ')}</small></span><input type="checkbox" name="module-${module.id}" ${module.enabled ? 'checked' : ''}></label>`).join('')}</div><div class="appearance-sample"><b>Aperçu immédiat</b><span>Navigation · liste · texte</span><button type="button" class="primary compact">Action</button></div><div class="button-row"><button class="primary" type="submit">Enregistrer le profil</button><button class="ghost" type="button" data-action="export-profile">Exporter</button><button class="ghost" type="button" data-action="restart-onboarding">Relancer l’assistant de configuration</button><label class="file-action">Importer<input id="profile-import" type="file" accept=".yaml,.streamdashboard.yaml,text/yaml"></label></div></form>`;
}
function onboarding() {
  if (!productProfile || productProfile.onboarding.completed) return '';
  const steps = ['Bienvenue','Profil','Modules','OBS','Twitch','Services optionnels','Apparence','Terminé']; const step = steps[onboardingStep] || steps[0]; const a = productProfile.appearance;
  const content = onboardingStep === 0 ? '<p>Quelques choix suffisent. Tout pourra être modifié plus tard.</p>'
    : onboardingStep === 1 ? `<label>Ton nom<input name="displayName" maxlength="80" value="${esc(productProfile.profile.displayName)}"></label><label>Ta chaîne<input name="channelName" maxlength="80" value="${esc(productProfile.profile.channelName)}"></label>`
    : onboardingStep === 2 ? `<div class="module-list">${moduleStates.filter(module => !['googleCalendar','discord','streamlabs','wizebot'].includes(module.id)).map(module => `<label class="switch"><span><b>${esc(module.label)}</b></span><input type="checkbox" name="module-${module.id}" ${productProfile.modules[module.id] ? 'checked' : ''}></label>`).join('')}</div>`
    : onboardingStep === 3 ? '<p>OBS reste local. Tu peux renseigner son adresse et tester la connexion dans Réglages, maintenant ou plus tard.</p>'
    : onboardingStep === 4 ? `<p>${connection('twitch').status === 'unavailable' ? 'Mode officiel — configuration mainteneur requise.' : 'La connexion Twitch peut être faite maintenant ou plus tard depuis Connexions.'}</p>`
    : onboardingStep === 5 ? `<p>Ces services sont facultatifs.</p>${['googleCalendar','discord','streamlabs','wizebot'].map(id => `<label class="switch"><span><b>${esc(moduleStates.find(module => module.id === id)?.label || id)}</b></span><input type="checkbox" name="module-${id}" ${productProfile.modules[id] ? 'checked' : ''}></label>`).join('')}`
    : onboardingStep === 6 ? `<div class="form-grid"><label>Thème<select name="theme">${['system','light','dark','oled'].map(value => `<option value="${value}" ${a.theme === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>Couleur<input name="accentColor" type="color" value="${esc(a.accent)}"></label><label>Densité<select name="density">${['comfort','normal','compact'].map(value => `<option value="${value}" ${a.density === value ? 'selected' : ''}>${value}</option>`).join('')}</select></label></div>`
    : '<p>StreamDashboard est prêt. Les services ignorés restent configurables dans Réglages.</p>';
  return `<dialog id="onboarding" class="onboarding"><form id="onboarding-form"><span class="label">ÉTAPE ${onboardingStep + 1} / ${steps.length}</span><h2>${step}</h2>${content}<div class="button-row">${onboardingStep ? '<button type="button" class="ghost" data-onboarding="previous">Retour</button>' : ''}<button type="button" class="ghost" data-onboarding="later">Configurer plus tard</button><button type="submit" class="primary">${onboardingStep === steps.length - 1 ? 'Terminer' : 'Continuer'}</button></div></form></dialog>`;
}
function settings() {
...
document.addEventListener('click', event => {
  ...
  const actions = {
    ...
    'export-profile': () => { location.href = '/api/v1/profile/export'; },
    'restart-onboarding': async () => { productProfile.onboarding.completed = false; onboardingStep = 0; await request('/api/v1/profile', 'PUT', productProfile); render(); },
  };
