...
async function refresh(force = false) { const [next, product] = await Promise.all([request('/api/v1/state'), request('/api/v1/profile')]); productProfile = product.profile; moduleStates = product.modules; applyProductAppearance(); applyStateUpdate(next, force); }
...
function nav() {
  const visiblePages = pages.filter(([id]) => id !== 'planning' || moduleEnabled('planning')).filter(([id]) => id !== 'prepare' || moduleEnabled('checklist') || moduleEnabled('notes') || moduleEnabled('templates'));
  ...
  const pageModules = { live: ['obs','twitch'], deck: ['obs'], fun: ['obs'], supports: ['streamlabs'], automations: ['automations'] }; document.querySelectorAll('.secondary-nav [data-page]').forEach(button => { const required = pageModules[button.dataset.page]; button.hidden = required ? !required.some(moduleEnabled) : false; });
...
function connections() {
  ...
  return `<div class="intent-panel"><span class="label">CONNEXIONS</span><h3>Services connectés</h3><div class="connection-cards">
    ${visible('obs') ? `<article><div><b>OBS</b><span>${state.obs.connected ? '● Connecté' : '○ Déconnecté'}</span></div><button class="secondary compact" data-action="go" data-value="settings">CONFIGURER</button></article>` : ''}
    ${visible('twitch') ? `<article><div><b>Twitch</b><span>${state.twitch.connected ? '● Connecté' : '○ Déconnecté'}</span></div><button class="secondary compact" data-action="${state.twitch.connected ? 'disconnect-twitch' : 'connect-twitch'}">${state.twitch.connected ? 'DÉCONNECTER' : 'CONNECTER'}</button></article>` : ''}
    ${visible('googleCalendar') ? `<article><div><b>Google Calendar</b><span>${state.google.connected ? '● Connecté' : '○ Déconnecté'}</span></div><button class="secondary compact" data-action="${state.google.connected ? 'disconnect-google' : 'connect-google'}">${state.google.connected ? 'DÉCONNECTER' : 'CONNECTER'}</button></article>` : ''}
    ${visible('discord') ? `<article><div><b>Discord</b><span>${state.discord?.connected ? '● Connecté' : state.discord?.configured ? '○ Erreur' : '○ À configurer'}</span></div><button class="secondary compact" data-action="go" data-value="settings">CONFIGURER</button></article>` : ''}
    ...
    ${visible('wizebot') ? `<article><div><b>WizeBot</b><span>${wizebot.status === 'CONNECTED' ? '●' : '○'} ${connectionLabel(wizebot.status)}${wizebot.profile?.name ? ` · ${esc(wizebot.profile.name)}` : ''}</span></div><form id="wizebot-form"><input name="apiBaseUrl" type="url" required aria-label="Adresse API fournie par WizeBot" placeholder="Adresse API fournie par WizeBot"><input name="token" type="password" maxlength="1000" required autocomplete="new-password" aria-label="Token WizeBot" placeholder="Token API"><div class="button-row"><button class="primary compact" type="submit">CONFIGURER</button><button class="secondary compact" type="button" data-action="refresh-wizebot">RAFRAÎCHIR</button><button class="danger compact" type="button" data-action="disconnect-wizebot">DÉCONNECTER</button></div></form></article>` : ''}
...
function bindForms() {
  ...
  if (profileForm) { profileForm.oninput = () => { const form = new FormData(profileForm); productProfile.appearance = { theme: form.get('theme'), preset: form.get('preset'), accent: form.get('accentColor'), density: form.get('density'), radius: form.get('radius'), textScale: form.get('textScale') }; applyProductAppearance(); }; profileForm.onsubmit = async event => { event.preventDefault(); const form = new FormData(profileForm); productProfile.profile.displayName = String(form.get('displayName') || 'Streamer'); productProfile.profile.channelName = String(form.get('channelName') || ''); for (const module of moduleStates) productProfile.modules[module.id] = form.get(`module-${module.id}`) === 'on'; for (const module of moduleStates.filter(item => productProfile.modules[item.id])) for (const dependency of module.dependencies) productProfile.modules[dependency] = true; for (const id of Object.keys(productProfile.providers)) productProfile.providers[id].mode = form.get(`provider-${id}`); const result = await request('/api/v1/profile', 'PUT', productProfile); productProfile = result.profile; moduleStates = result.modules; nav(); render(); toast('Profil enregistré'); }; }
  ...
  const onboardingForm = $('#onboarding-form'); if (onboardingForm) onboardingForm.onsubmit = async event => { event.preventDefault(); productProfile.onboarding.completed = true; await request('/api/v1/profile', 'PUT', productProfile); $('#onboarding').close(); page = 'settings'; nav(); render(); };
...
document.addEventListener('click', event => {
  ...
  const actions = {
    ...
    'skip-onboarding': async () => { productProfile.onboarding.completed = true; await request('/api/v1/profile', 'PUT', productProfile); $('#onboarding')?.close(); },
  };
