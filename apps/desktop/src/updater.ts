...
export function startUpdater(window: BrowserWindow, dashboard: DashboardServerHandle, logger: Pick<Console, 'info' | 'error'> = console) {
  ...
  const firstCheck = setTimeout(() => autoUpdater.checkForUpdates(), process.argv.includes('--squirrel-firstrun') ? 10_000 : 0);
  ...
  return () => { clearTimeout(firstCheck); clearInterval(check); clearInterval(readyWatcher); };
}
