...
export async function startDesktopRuntime(): Promise<DesktopRuntime> {
  ...
  try { const config = JSON.parse(await readFile(path.join(userData, 'config', 'dashboard.json'), 'utf8')); launchObs = config.settings?.launchObs === true; obsExecutablePath = typeof config.settings?.obsExecutablePath === 'string' ? config.settings.obsExecutablePath : undefined; } catch { /* first run */ }
  const obsResult = await launchObsIfRequested(launchObs, obsExecutablePath); logger.info(obsResult.detail);
  ...
  let stopped = false;
  return { dashboard, logger, ensureObsRunning: () => launchObsIfRequested(true, dashboard.state().settings.obsExecutablePath), stop: async () => { if (stopped) return; stopped = true; await dashboard.stop(); logger.info('Arrêt propre terminé'); } };
}
