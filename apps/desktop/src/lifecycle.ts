...
export async function startDesktopRuntime(): Promise<DesktopRuntime> {
  ...
  let stopped = false;
  return { dashboard, logger, ensureObsRunning: () => launchObsIfRequested(true), stop: async () => { if (stopped) return; stopped = true; await dashboard.stop(); logger.info('Arrêt propre terminé'); } };
}
